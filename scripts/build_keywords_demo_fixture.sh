...
FIXTURE_MANIFEST="$ROOT_DIR/fixtures/keywords-demo/native-lib/Cargo.toml"
TARGET_DIR="$ROOT_DIR/target/keywords-demo-native"
...
case "$(uname -s)" in
  Darwin)
    LIB_FILE="libuniffi_keywords_demo.dylib"
    ;;
  Linux)
    LIB_FILE="libuniffi_keywords_demo.so"
    ;;
  MINGW*|MSYS*|CYGWIN*)
    LIB_FILE="uniffi_keywords_demo.dll"
    ;;
  *)
