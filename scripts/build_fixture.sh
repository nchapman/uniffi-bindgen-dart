...
FIXTURE_MANIFEST="$ROOT_DIR/fixtures/simple-fns/native-lib/Cargo.toml"
TARGET_DIR="$ROOT_DIR/target/simple-fns-native"
...
case "$(uname -s)" in
  Darwin)
    LIB_FILE="libuniffi_simple_fns.dylib"
    ;;
  Linux)
    LIB_FILE="libuniffi_simple_fns.so"
    ;;
  MINGW*|MSYS*|CYGWIN*)
    LIB_FILE="uniffi_simple_fns.dll"
    ;;
  *)
