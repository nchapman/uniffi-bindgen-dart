...
FIXTURE_MANIFEST="$ROOT_DIR/fixtures/compound-demo/native-lib/Cargo.toml"
TARGET_DIR="$ROOT_DIR/target/compound-demo-native"
...
case "$(uname -s)" in
  Darwin)
    LIB_FILE="libuniffi_compound_demo.dylib"
    ;;
  Linux)
    LIB_FILE="libuniffi_compound_demo.so"
    ;;
  MINGW*|MSYS*|CYGWIN*)
    LIB_FILE="uniffi_compound_demo.dll"
    ;;
  *)
