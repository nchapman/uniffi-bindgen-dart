...
/// Renders all object constructor and method bindings (the `for object in objects` loop).
pub(super) fn render_object_members(out: &mut String, ctx: &RenderMethodContext) {
    let ffi_namespace = ctx.ffi_namespace;
    let local_module_path = ctx.local_module_path;
        ...
        for ctor in &object.constructors {
            if let Some(reason) = ctor.runtime_unsupported.as_ref() {
                ...
                if ffibuffer_eligible {
                        ...
                        .zip(arg_ffi_offsets.iter())
                    {
                        match ffi_type {
                            FfiType::RustBuffer(_) => {
                                super::super::ffi_buffer::render_ffibuffer_rustbuffer_arg_serialization(
                                    out,
                                    arg,
                                    *offset,
                                    &escaped_reason,
                            ...
                            _ => {
                                super::super::ffi_buffer::render_ffibuffer_primitive_arg_write(
                                    out,
                                    arg,
                                    ffi_type,
                                    *offset,
                    ...
                    super::super::ffi_buffer::render_ffibuffer_outer_cleanup(out);
                    out.push_str("  }\n");
                    continue;
                }
                out.push('\n');
        ...
        for method in &object.methods {
            if let Some(reason) = method.runtime_unsupported.as_ref() {
                ...
                let async_method_eligible =
                    super::super::ffi_buffer::is_runtime_unsupported_async_ffibuffer_eligible_method(method);
                if async_method_eligible {
                    ...
                    // Poll loop.
                    render_ffibuffer_async_poll_loop(out, &poll_field, &method.name);
