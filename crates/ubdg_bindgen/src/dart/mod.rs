...
impl UdlFunction {
    ...
    fn uses_runtime_bytes(&self) -> bool {
        self.return_type
            ...
            || self.args.iter().any(|a| is_runtime_bytes_like_type(&a.type_))
    }
...
fn render_dart_scaffold(
    ...
    let needs_runtime_optional_bytes = functions.iter().any(|f| {
        is_runtime_ffi_compatible_function(f, records, enums)
            && (f.return_type.as_ref().is_some_and(is_runtime_optional_bytes_type)
                || f.args.iter().any(|a| is_runtime_optional_bytes_type(&a.type_)))
    });
    let needs_runtime_sequence_bytes = functions.iter().any(|f| {
        is_runtime_ffi_compatible_function(f, records, enums)
            && (f.return_type.as_ref().is_some_and(is_runtime_sequence_bytes_type)
                || f.args.iter().any(|a| is_runtime_sequence_bytes_type(&a.type_)))
    });
    ...
    out.push_str("}\n");
    out.push_str(&render_function_stubs(functions, ffi_class_name, records, enums));
    out
}
...
fn render_data_models(records: &[UdlRecord], enums: &[UdlEnum]) -> String {
    ...
    for record in records {
        ...
        out.push_str("    };\n");
        out.push_str("  }\n\n");
        out.push_str(&format!("  static {class_name} fromJson(Map<String, dynamic> json) {{\n"));
...
fn render_json_decode_expr(value_expr: &str, type_: &Type) -> String {
    match type_ {
        ...
        Type::Timestamp => format!("DateTime.fromMicrosecondsSinceEpoch(({value_expr} as num).toInt(), isUtc: true)"),
...
fn render_bound_methods(functions: &[UdlFunction], records: &[UdlRecord], enums: &[UdlEnum]) -> String {
    let mut out = String::new();
    ...
    let needs_bytes_free = functions
        .iter()
        .any(|f| is_runtime_ffi_compatible_function(f, records, enums) && f.returns_runtime_bytes());
    let needs_bytes_vec_free = functions
        .iter()
        .any(|f| is_runtime_ffi_compatible_function(f, records, enums) && f.return_type.as_ref().is_some_and(is_runtime_sequence_bytes_type));
    ...
    for function in functions {
        ...
        let native_return = function
            .return_type
            .as_ref()
            .map(|t| map_runtime_native_ffi_type(t, records, enums))
            .unwrap_or(Some("ffi.Void"));
        let dart_ffi_return = function
            .return_type
            .as_ref()
            .map(|t| map_runtime_dart_ffi_type(t, records, enums))
            .unwrap_or(Some("void"));
        ...
        match function.return_type.as_ref() {
            ...
            Some(type_) if is_runtime_sequence_bytes_type(type_) => {
                ...
                out.push_str("      final ffi.Pointer<_RustBuffer> resultData = resultVec.data;\n");
                ...
                out.push_str("          }\n");
                out.push_str("          try {\n");
                out.push_str("            out.add(Uint8List.fromList(itemData.asTypedList(itemLen)));\n");
                out.push_str("          } finally {\n");
