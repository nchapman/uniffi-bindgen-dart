...
fn render_dart_scaffold(ctx: &RenderContext<'_>) -> String {
    ...
    let has_runtime_unsupported = functions.iter().any(|f| f.runtime_unsupported.is_some())
        ...
        || enums
            ...
            .any(|m| m.runtime_unsupported.is_some());
    let has_non_string_map_boundary = {
        let arg_is_non_string_map = |a: &UdlArg| is_runtime_non_string_map_type(&a.type_);
        // UdlObjectMethod is reused for object, record, and enum methods.
        let method_uses_non_string_map = |m: &UdlObjectMethod| {
            m.return_type
                .as_ref()
                .is_some_and(is_runtime_non_string_map_type)
                || m.args.iter().any(arg_is_non_string_map)
        };
        functions.iter().any(|f| {
            f.return_type
                .as_ref()
                .is_some_and(is_runtime_non_string_map_type)
                || f.args.iter().any(arg_is_non_string_map)
        }) || objects
            .iter()
            .any(|o| o.methods.iter().any(method_uses_non_string_map))
            || records
                .iter()
                .flat_map(|r| r.methods.iter())
                .any(method_uses_non_string_map)
            || enums
                .iter()
                .flat_map(|e| e.methods.iter())
                .any(method_uses_non_string_map)
    };
    let needs_binary_helpers = has_runtime_unsupported || has_non_string_map_boundary;
