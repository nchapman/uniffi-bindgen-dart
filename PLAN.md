...
- Runtime/custom-type unwrapping for builtin-backed custom typedefs (currently validated for string-backed aliases) across sync/async marshalling and Rust-future completion decoding.
...
| Async futures | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | `Future<T>`-based idiomatic APIs | top-level/object API wrappers for `[Async]` with return-type driven rust-future symbol selection, including bytes/optional-bytes/bytes-sequence/string-keyed-map completion families and builtin-backed custom typedef unwrapping | rust-future poll/cancel/complete/free flow implemented for string, `void`, integer, bytes, optional-bytes, bytes-sequence, and string-keyed-map fixture paths for top-level + object methods | add dedicated futures stress fixture and complete remaining external/custom return families | In progress | async lifecycle is runtime-driven (poll/wake/ready/cancel/free); map/custom async paths are now validated in generator unit tests, goldens, and runtime smoke tests |
...
| Custom types | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | Dart conversion helpers | builtin-backed typedef unwrapping in type mapping + marshalling is implemented for runtime-compatible paths | fixture/runtime coverage currently validates string-backed custom typedefs in sync/async calls | add dedicated custom-types fixture for broader lift/lower families (non-string, nested/container forms) | In progress | current coverage proves builtin-backed custom typedef parity for string family; generalized custom conversion matrix still pending |
...
3. Add a dedicated futures stress fixture to harden poll/wake/cancel/free behavior across broader async type families and error/cancellation edge cases.
4. Keep `docs/supported-features.md` synchronized with every parity change.
5. Extend remaining custom/external parity (non-string custom lift/lower families + cross-crate/ext-type scenarios).
