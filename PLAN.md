...
| Custom types | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | Dart conversion helpers | builtin-backed typedef unwrapping in type mapping + marshalling is implemented for runtime-compatible paths | fixture/runtime coverage now validates string, `u32`, and bytes/optional-bytes custom typedefs in sync/async top-level + object calls | add dedicated custom-types fixture for broader lift/lower families (nested/container forms and non-runtime-compatible conversions) | In progress | current coverage proves builtin-backed custom typedef parity for primary runtime families; generalized custom conversion matrix still pending |
