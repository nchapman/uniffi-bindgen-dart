...
| Regression rows (`regressions/*`) | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | N/A | policy in place; dedicated regression fixture coverage now includes `custom-shadow-demo` golden guard | policy in place | `golden_regression_custom_shadow_demo`; fixture `fixtures/regressions/custom-shadow-demo` | In progress | regression fixture now guards nested custom JSON closure shadowing; continue adding one row per bug class |
