...
- Phase 5: Async Rust-future ABI now covers string, `void`, and integer fixture paths; sync callback interface top-level function-argument support now includes primitive + string/optional string/record/enum method types, while async/throwing callback parity remains.
...
- Callback interface bridge support for sync top-level function-argument callbacks, including primitive + string/optional string/record/enum callback method marshalling, with fixture/runtime verification.
...
| Callback interfaces (sync/async) | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | callback APIs with Dart callable conventions | sync callback interface model + vtable bridge generation for top-level functions with primitive + string/optional string/record/enum callback method shapes | fixture native library supports callback-vtable init + callback invocation lifecycle (`clone`/`free` + method dispatch) across adder + formatter callback paths | `dart::tests::renders_runtime_callback_interface_bindings`; golden `simple-fns`; `binding_tests/test/smoke_test.dart` callback runtime assertions | In progress | sync callback path is green for covered method types; async/throws/object-method callback parity still pending |
...
1. Extend callback interfaces from current sync top-level coverage to async/throws paths and object-method parity.
