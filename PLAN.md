...
- Phase 5: Async Rust-future ABI now covers string, `void`, integer, bytes, optional-bytes, and bytes-sequence fixture paths for top-level functions and object methods; callback support now covers sync/async/throwing top-level functions and object methods, including callback-interface methods with async/throws for primitive + string/optional string/record/enum return families with runtime fixture validation.
...
- Async `[Async]` wrappers and Rust-future poll/cancel/complete/free lifecycle coverage across string, `void`, integer, bytes, optional-bytes, and bytes-sequence fixture paths.
...
| Async futures | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | `Future<T>`-based idiomatic APIs | top-level/object API wrappers for `[Async]` with return-type driven rust-future symbol selection, including bytes/optional-bytes/bytes-sequence completion families | rust-future poll/cancel/complete/free flow implemented for string, `void`, integer, bytes, optional-bytes, and bytes-sequence fixture paths | extend to remaining return-type families (maps/custom/external) + dedicated futures fixture stress cases | In progress | async lifecycle is runtime-driven (poll/wake/ready/cancel/free); coverage expanded and validated in generator/golden/runtime smoke tests |
...
3. Extend async Rust-future parity to remaining return-type families (maps/custom/external) with dedicated futures stress fixtures.
