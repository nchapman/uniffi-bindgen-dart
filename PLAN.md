...
### In Progress
- Phase 4: Enums/errors are in place; trait parity now covers object-level `Display`/`Debug`/`Hash`/`Eq` mapping; `Ord` UDL parsing is available in the current UniFFI toolchain (`0.31.x`) and Dart-side idiomatic mapping remains to be implemented.
...
2. Implement Dart-side idiomatic `Ord` parity (`Comparable<T>`/`compareTo` mapping and tests) now that UniFFI `0.31.x` parser support is confirmed.
