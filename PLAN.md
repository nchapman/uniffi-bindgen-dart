...
- Phase 6: Advanced config is partially implemented (`rename`/`exclude` for generated public API wrappers with dedicated `rename-demo` golden coverage) and docstring emission is now in place with dedicated `docstrings-demo` coverage; external type support is now partially implemented for external-record typedef wrapper binding plus `external_packages` import mapping, with broader external parity still pending.
...
- External record typedef wrapper generation is now wired with `external_packages` import mapping and deterministic `ext-types-demo` golden coverage.
...
| External/remote types | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | cross-package import/type mapping | `external_packages` config + external crate usage import emission implemented; external record + enum typedefs are treated as runtime-compatible pointer/JSON paths; stable public `*FfiCodec` helpers now provide enum/error cross-package conversion contracts | dedicated `ext-types-demo` golden validates external-record + external-enum wrapper generation and package import mapping; full cross-crate runtime fixture matrix is still pending | `dart::tests::imports_external_package_and_binds_external_record_and_enum_types`; golden `ext-types-demo` | In progress | UniFFI removed `Type::External`; current path infers externality from `module_path` crate segment; broader external error/object/trait parity still pending deeper metadata/library-mode integration |
