...
| External/remote types | `/Users/nchapman/Drive/Code/lessisbetter/refs/uniffi-rs` | cross-package import/type mapping | `external_packages` config + external crate usage import emission implemented; external record typedefs now treated as runtime-compatible pointer/JSON paths | dedicated `ext-types-demo` golden validates external-record wrapper generation and package import mapping; full cross-crate runtime fixture matrix is still pending | `dart::tests::imports_external_package_and_binds_external_record_type`; golden `ext-types-demo` | In progress | UniFFI removed `Type::External`; current path infers externality from `module_path` crate segment; broader external enum/error/object/trait parity still pending deeper metadata/library-mode integration |
