...
| Objects/interfaces | Partial | sync constructors/methods + lifecycle (`close`/finalizer) implemented; async wrappers for `[Async]` methods are generated across currently supported return families; trait edge-case parity still pending |
...
| Async futures | Partial | `[Async]` maps to idiomatic `Future<...>` APIs with rust-future poll/cancel/complete/free runtime flow for string, `void`, integer, bytes, optional-bytes, and bytes-sequence return families; maps/custom/external async parity still pending |
