...
| External/remote types | Partial | external record typedef and external enum typedef references bind through runtime wrappers with mapped `external_packages` imports; generator now emits stable public `*FfiCodec` helpers for enum/error cross-package conversion contracts; dedicated `ext-types-demo` golden coverage is in place; full external parity (errors/objects/traits across crates) still needs deeper metadata/library-mode integration |
